...
import { calculateAge, isValid } from '../screens/Dashboard/chat/lib/utils';
...
const nav = useNavigate() 
 ...
 const fetchImages = async () => {
  ...
  const response = await fetch(`${baseUrl}/api/master/profile-images`);
...
const imageArr = imagesArray?.filter((d) => d?.appUserId == profile?.id)

   return  (<div className="group relative bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 h-full flex flex-col">
      ...
      20 % Match
        ...
        {/* Online Status */}
        <div className={`absolute top-3 right-3 text-xs font-medium px-2 py-1 rounded-full  bg-gray-200 text-gray-700`}>
      ...
      <div className="p-4 md:p-5 flex-grow flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-1 md:mb-2">
            <h3 className="text-base md:text-lg font-bold text-gray-800">
              {profile?.email}, <span className="text-gray-600">{calculateAge(profile?.age)}</span>
            </h3>
          </div>

          <div className="flex items-center text-gray-600 mb-1 md:mb-2">
            <FaBriefcase className="mr-2 text-secondary text-sm" />
            <span className="text-xs md:text-sm">{profile?.occupation?.name}</span>
          </div>

          <div className="flex items-center text-gray-600 mb-2 md:mb-3">
            <FaMapMarkerAlt className="mr-2 text-secondary text-sm" />
            <span className="text-xs md:text-sm">{`${JSON.parse(profile?.city)?.name}`}</span>
          </div>
        ...
        <button onClick={() => nav("/dashboard/client-profile",{state: profile})} className="hidden md:flex w-full bg-gradient-to-r from-secondary to-secondarydark text-white py-2 rounded-lg hover:opacity-90 transition-all duration-300 items-center justify-center text-sm font-medium">
          <FaUser className="mr-2" />
          View Full Profile
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-primary mx-auto max-w-7xl">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Call to Action */}
        <div className="mt-12 text-center">
          <button onClick={() => nav("/browse")} className="px-6 py-2 bg-white border-2 border-secondary text-secondary rounded-full hover:bg-secondary hover:text-white transition-all duration-300 font-medium shadow-md hover:shadow-purple-200 text-sm">
            View All Profiles
          </button>
        </div>
      </div>
...
export default ProfilesCorousel;